...
func (r *Exprec) Save_i(key string, i int, keys []string, all bool) {
	...
	if all == false {
		//
		if Contains(keys, key) == false { return }
	}
...
func (r *Exprec) Load(keys []string, all bool) bool {
  ...
  //
  if f("drug") { r.Drug, _ = redis.String(_s.Do("hget", r.Varname, "drug")) }
  if f("bolus") { r.Bolus, _ = redis.Int(_s.Do("hget", r.Varname, "bolus")) }
  if f("infusion") { r.Infusion, _ = redis.Int(_s.Do("hget", r.Varname, "infusion")) }

  //
  if f("weight") { r.Weight, _ = redis.Int(_s.Do("hget", r.Varname, "weight")) }
  if f("age") { r.Age, _ = redis.Int(_s.Do("hget", r.Varname, "age")) }

  if f("mtime") { r.Mtime, _ = redis.Int(_s.Do("hget", r.Varname, "mtime")) }
  if f("cycle") { r.Cycle, _ = redis.Int(_s.Do("hget", r.Varname, "cycle")) }

	if f("unitVd") { r.UnitVd, _ = redis.Int(_s.Do("hget", r.Varname, "unitVd")) }
	if f("absoluteVd") { r.AbsoluteVd, _ = redis.Int(_s.Do("hget", r.Varname, "absoluteVd")) }
	if f("targetTOF") { r.TargetTOF, _ = redis.Int(_s.Do("hget", r.Varname, "targetTOF")) }
	if f("targetPTC") { r.TargetPTC, _ = redis.Int(_s.Do("hget", r.Varname, "targetPTC")) }
	if f("EC50") { r.EC50, _ = redis.Float64(_s.Do("hget", r.Varname, "EC50")) }
	if f("TOF") { r.TOF, _ = redis.Int(_s.Do("hget", r.Varname, "TOF")) }
	if f("PTC") { r.PTC, _ = redis.Int(_s.Do("hget", r.Varname, "PTC")) }
